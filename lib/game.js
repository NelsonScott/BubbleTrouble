(function(){
  ...
  Game.prototype.checkCollisions = function(){
    ...
    // objs.forEach(function(first){
    //   objs.forEach(function(second){
    //       if (first == second){
    //         return;
    //       }
    //
  ...
  Game.prototype.addBubbles = function(options){
    var pos, vel, mult, bubble;

    for (var i = 0; i < options.NumOfBubbles; i++){
      pos = options.posArr[i];
      vel = options.vels[i];
      mult = options.bubbleMultiplier;
      this.addObject(new BubbleTrouble.Bubble(pos, vel, this.makeRadius(mult), this));
    }
  }
